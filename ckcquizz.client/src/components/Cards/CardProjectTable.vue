<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
		...
		<a-table :columns="columns" :data-source="data" :pagination="false">
			...
			<!-- FIX: Slot syntax for Ant Design Vue 3+ -->
			<template #bodyCell="{ column, record }">
				<template v-if="column.key === 'members'">
					<a-space :size="-12" class="avatar-chips">
						<!-- FIX: :key moved to the <template> tag -->
						<template v-for="memberSrc in record.members" :key="memberSrc">
							<a-avatar size="small" :src="memberSrc" />
						</template>
					</a-space>
				</template>
				<template v-else-if="column.key === 'company'">
					<h6 class="m-0">
						<img :src="record.company.logo" width="25" class="mr-10" :alt="record.company.name">
						{{ record.company.name }}
					</h6>
				</template>
				<template v-else-if="column.key === 'completion'">
					...
					<a-progress :percent="record.completion.value ? record.completion.value : record.completion" :show-info="false" size="small" :status="record.completion.status ? record.completion.status : 'normal'" />
				</template>
			</template>
