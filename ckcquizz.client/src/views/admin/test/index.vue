<template>
  <a-card title="Quản lý Đề kiểm tra" style="width: 100%">
        ...
        <template v-if="column.key === 'actions'">
          <a-space>
            ...
            <a-tooltip title="Soạn câu hỏi">
              <a-button type="text" @click="openQuestionComposer(record)">
                <template #icon>
                  <FilePlus2 />
                </template>
              </a-button>
    ...
    <!-- 3. Modal Thêm/Sửa -->
    <a-modal :title="modalState.isEditMode ? 'Sửa Đề thi' : 'Tạo Đề thi mới'"
             :open="modalState.show"
             @ok="handleSubmit"
             @cancel="handleCancel"
             :confirmLoading="modalState.isSaving"
             width="900px"
             destroyOnClose>
            ...
            <div :class="{ 'disabled-section': modalState.isEditMode }">
              ...
              <div v-if="isUsingQuestionBank">
                <a-form-item label="Chọn chương" name="machuongs">
                  <a-select v-model:value="formState.machuongs"
                            mode="multiple"
                            placeholder="Chọn các chương"
                            :options="dropdownData.chuongOptions"
                            :loading="dropdownData.isLoading" />
                </a-form-item>
              </div>
    ...
    <QuestionComposerModal v-if="composerState.selectedDeThi"
                           :open="composerState.isVisible"
                           :deThi="composerState.selectedDeThi"
                           @update:open="composerState.isVisible = false"
