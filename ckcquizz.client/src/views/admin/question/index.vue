<template>
  <div class="page-container">
    <a-card>
        ...
        <a-row :gutter="[16, 16]">
          ...
          <a-col :span="6">
            <a-select v-model:value="filters.maChuong" placeholder="Chọn chương" style="width: 100%" :disabled="!filters.maMonHoc" allow-clear @change="handleFilterChange">
              <a-select-option v-for="chapter in chapters" :key="chapter.machuong" :value="chapter.machuong">{{ chapter.tenchuong }}</a-select-option>
            </a-select>
          </a-col>
          <a-col :span="6">
      ...
      <!-- ========================================================== -->
      <a-table :columns="columns" :data-source="dataSource" :pagination="pagination" :loading="Modalloading" @change="handleTableChange" row-key="macauhoi">
    ...
    <!-- ========================================================== -->
    <a-modal v-model:open="isAddModalVisible" title="Thêm câu hỏi mới" width="800px" :confirm-loading="addModalLoading" ok-text="Thêm" cancel-text="Hủy" @ok="handleAddOk" @cancel="handleAddCancel" :destroyOnClose="true">
      <a-form ref="addFormRef" :model="addFormState" :rules="formRules" layout="vertical">
        <a-row :gutter="16">
          <a-col :span="12"><a-form-item label="Môn học" name="maMonHoc"><a-select v-model:value="addFormState.maMonHoc" placeholder="Chọn môn học"><a-select-option v-for="subject in subjects" :key="subject.mamonhoc" :value="subject.mamonhoc">{{ subject.tenmonhoc }}</a-select-option></a-select></a-form-item></a-col>
    ...
    <!-- MODAL SỬA CÂU HỎI -->
    <!-- ========================================================== -->
    <a-modal v-model:open="isEditModalVisible" title="Chỉnh sửa câu hỏi" width="800px" :confirm-loading="editModalLoading" ok-text="Lưu" cancel-text="Hủy" @ok="handleEditOk" @cancel="handleEditCancel" :destroyOnClose="true">
      <a-spin :spinning="editModalLoading">
        <a-form v-if="!editModalLoading" ref="editFormRef" :model="editFormState" :rules="formRules" layout="vertical">
          <a-row :gutter="16">
            <a-col :span="12"><a-form-item label="Môn học" name="maMonHoc"><a-select v-model:value="editFormState.maMonHoc" placeholder="Chọn môn học"><a-select-option v-for="subject in subjects" :key="subject.mamonhoc" :value="subject.mamonhoc">{{ subject.tenmonhoc }}</a-select-option></a-select></a-form-item></a-col>
          ...
          <dynamic-form-elements :formState="editFormState" :form-ref="editFormRef" @update:file-list="editFormState.fileList = $event" />

          <a-row style="margin-top: 16px;"><a-col :span="24"><a-form-item label="Trạng thái" name="trangthai"><a-switch v-model:checked="editFormState.trangthai" checked-children="Hiển thị" un-checked-children="Ẩn" /></a-form-item></a-col></a-row>
        </a-form>
      </a-spin>
    </a-modal>
  </div>
